...
function Navbar() {
    ...
    return (
        <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all z-50">
            ...
            <NavLink
                to="/seller"
                className="hidden lg:inline-block px-5 py-2 border border-primary text-primary font-medium rounded-full hover:bg-primary hover:text-white transition-all duration-300"
                >
                Seller Login
            </NavLink>
            ...
            { open && (
                <div className={`${open ? 'flex' : 'hidden'} absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden`}>
                    ...
                    {/* <NavLink to='/seller' onClick={() => setOpen(false)}>Seller Login</NavLink> */}
